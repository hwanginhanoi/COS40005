...
def crawl_domain_mogi_hn():
    start_page = 20300
    total_pages = 200
    num_ranges = 5 #threads
    tasks = []

    # Calculate the number of pages per range
    pages_per_range = total_pages // num_ranges

    # Dynamically create the ranges
    for i in range(num_ranges):
        range_start = start_page + i * pages_per_range
        range_end = range_start + pages_per_range

        # Handle any remaining pages in the last range
        if i == num_ranges - 1:
            range_end = start_page + total_pages

        print("Starting range:", range_start, "-", range_end)

        # Create subtasks for each range
        tasks.append(crawl_domain_mogi_hn_subtask.s(range_start, range_end))

    group(tasks).apply_async()

@shared_task()
def crawl_domain_mogi_hn_subtask(range_start, range_end):
    ...
    driver = get_chrome_driver()
    page = range_start #thay start number cua m vao day
    while True:
        print("Current page number: ", page)
        if page > range_end: #lay start_page + 1k, thay vao day
            break
...
def crawl_domain_mogi_hcm():
    ...
    while True:
        print("Current page number: ", page)
        if page > 25000: #lay start_page + 1k, thay vao day
            break
        url = url_template.format(page=page)
...
NUMBER_OF_THREADS = 5 # Neu tren windows thi chinh len (5), macos chinh xuong (2)
@shared_task
def crawl_property(domain_names):
    ...
    for domain_name in domain_names:
        ...
        if total_caches > 0:
            # caches = list(Cache.objects.filter(domain=domain, status=False)[-1000:])

            total_caches = Cache.objects.filter(domain=domain, status=False).count()
            start_index = max(0, total_caches - 300)  # Calculate the starting index
            caches = list(Cache.objects.filter(domain=domain, status=False)[start_index:total_caches])
